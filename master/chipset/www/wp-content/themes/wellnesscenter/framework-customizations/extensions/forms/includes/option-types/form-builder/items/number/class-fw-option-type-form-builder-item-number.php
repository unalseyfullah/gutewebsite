...
class Wellnesscenter_Option_Type_Form_Builder_Item_Number extends FW_Option_Type_Form_Builder_Item {
	...
	public function get_thumbnails() {
		return array(
			array(
				'html' =>
					'<div class="item-type-icon-title" data-hover-tip="' . __( 'Add a Number field', 'wellnesscenter' ) . '">' .
					'<div class="item-type-icon"><img src="' . esc_url( $this->get_uri( '/static/images/icon.png' ) ) . '" /></div>' .
					'<div class="item-type-title">' . __( 'Number', 'wellnesscenter' ) . '</div>' .
					'</div>'
			)
		);
	}
	...
	public function enqueue_static() {
		...
		wp_localize_script(
			...
			array(
				...
				'l10n'     => array(
					'item_title'      => __( 'Number', 'wellnesscenter' ),
					...
					'options' => array(
						array(
							'label' => array(
								...
								'value' => __( 'Number', 'wellnesscenter' ),
							)
						),
						array(
							'required' => array(
							...
							'constraints' => array(
								...
								'picker'  => array(
									'constraint' => array(
										...
										'desc'    => __( 'Set digits or values restrictions of this field', 'wellnesscenter' ),
										'type'    => 'radio',
										'inline'  => true,
										'choices' => array(
											'digits' => __( 'Digits', 'wellnesscenter' ),
											'value'  => __( 'Value', 'wellnesscenter' )
										),
									)
								),
								'choices' => array(
	...
	public function frontend_validate( array $item, $input_value ) {
		...
		$messages = array(
			'invalid'             => str_replace(
				array( '{label}' ),
				array( $options['label'] ),
				__( 'The {label} field must be a valid number', 'wellnesscenter' )
			),
			...
			'digits_min_singular' => str_replace(
				array( '{label}' ),
				array( $options['label'] ),
				__( 'The {label} field must have minimum %d digit', 'wellnesscenter' )
			),
			'digits_min_plural'   => str_replace(
				array( '{label}' ),
				array( $options['label'] ),
				__( 'The {label} field must have minimum %d digits', 'wellnesscenter' )
			),
			'digits_max_singular' => str_replace(
				array( '{label}' ),
				array( $options['label'] ),
				__( 'The {label} field must have maximum %d digit', 'wellnesscenter' )
			),
			'digits_max_plural'   => str_replace(
				array( '{label}' ),
				array( $options['label'] ),
				__( 'The {label} field must have maximum %d digits', 'wellnesscenter' )
			),
			'value_min'           => str_replace(
				array( '{label}' ),
				array( $options['label'] ),
				__( 'The {label} field minimum value must be %s', 'wellnesscenter' )
			),
			'value_max'           => str_replace(
				array( '{label}' ),
				array( $options['label'] ),
				__( 'The {label} field maximum value must be %s', 'wellnesscenter' )
			),
		);
...
FW_Option_Type_Builder::register_item_type( 'Wellnesscenter_Option_Type_Form_Builder_Item_Number' );