	...
	elseif ( in_array( $field_type, array( 'verification', 'secret' ) ) ) :
		...
		if ( $field_type == 'secret' ) :
			...
			if ( is_user_logged_in() ) :
				...
				$verification .= sprintf(
					'<li class="vfb-item" id="li-%1$s">%2$s</li>',
					$id_attr,
					$logged_in_as
				);
			endif;
			...
			$verification .= sprintf(
				'<li class="vfb-item vfb-item-%1$s" style="display:%2$s"><label for="%3$s" class="vfb-desc">%4$s%5$s</label>',
				$field_type,
				$logged_in_display,
				$id_attr,
				$field_name,
	...
	switch ( $field_type ) {
		...
		case 'submit' :
			$submit = sprintf(
				...
				<input type="submit" name="vfb-submit-button" id="%2$s" value="%3$s" class="vfb-submit btn btn-primary %4$s" />
				</li>',
				$field_id,
				$id_attr,
