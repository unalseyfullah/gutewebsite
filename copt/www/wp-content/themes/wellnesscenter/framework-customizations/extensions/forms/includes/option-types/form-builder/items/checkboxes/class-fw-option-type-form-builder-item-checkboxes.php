...
class Wellnesscenter_Option_Type_Form_Builder_Item_Checkboxes extends FW_Option_Type_Form_Builder_Item {
		...
		return array(
			array(
				'html' =>
					...
					'</div>' .
					'<div class="item-type-title">' . __( 'Multiple Choice', 'wellnesscenter' ) . '</div>' .
					'</div>'
			)
		);
	}
	...
	public function enqueue_static() {
		...
		wp_localize_script(
			...
			array(
				'l10n'     => array(
					'item_title'      => __( 'Multiple Choice', 'wellnesscenter' ),
					...
					'x_more'          => __( '{x} More', 'wellnesscenter' ),
					'close'           => __( 'Close', 'wellnesscenter' ),
				),
	...
	private function get_options() {
		return array(
					...
					'options' => array(
						array(
							'label' => array(
								...
								'value' => __( 'Multiple Choice', 'wellnesscenter' ),
							)
						),
						array(
							'required' => array(
			...
			array(
				'layout' => array(
					'type'    => 'select',
					'label'   => __( 'Field Layout', 'wellnesscenter' ),
					'desc'    => __( 'Select choice display layout', 'wellnesscenter' ),
					'choices' => array(
						'one-column'    => __( 'One column', 'wellnesscenter' ),
						'two-columns'   => __( 'Two columns', 'wellnesscenter' ),
						'three-columns' => __( 'Three columns', 'wellnesscenter' ),
						'side-by-side'  => __( 'Side by side', 'wellnesscenter' ),
					),
				)
			),
			array(
	...
	protected function get_fixed_attributes( $attributes ) {
		...
		if ( empty( $attributes['options']['choices'] ) ) {
			$attributes['options']['choices'][] = __( 'Multiple Choice', 'wellnesscenter' );
		}
	...
	public function frontend_validate( array $item, $input_value ) {
		...
		$messages = array(
			...
			'not_existing_choices' => str_replace(
				array( '{label}' ),
				array( $options['label'] ),
				__( '{label}: Submitted data contains not existing choices', 'wellnesscenter' )
			),
		);
...
FW_Option_Type_Builder::register_item_type( 'Wellnesscenter_Option_Type_Form_Builder_Item_Checkboxes' );
