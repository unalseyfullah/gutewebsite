...
if ( ! class_exists( 'Wellnesscenter_Plugin_Activation' ) ) {
    class Wellnesscenter_Plugin_Activation {
         ...
         * @since 1.0.0
         *
         * @var Wellnesscenter_Plugin_Activation
         */
         ...
         * @var string
         */
        public $menu = 'wellnesscenter-install-plugins';
         ...
         * @var int
         */
        public $wellnesscenter_wp_version;

        /**
         * Adds a reference of this object to $instance, populates default strings,
         * does the wellnesscenter_init action hook, and hooks in the interactions to init.
         *
         * @since 1.0.0
         *
         * @see Wellnesscenter_Plugin_Activation::init()
         */
        public function __construct() {
            ...
            $this->strings = array(
                'page_title'                     => __( 'Install Required Plugins', 'wellnesscenter' ),
                'menu_title'                     => __( 'Install Plugins', 'wellnesscenter' ),
                'installing'                     => __( 'Installing Plugin: %s', 'wellnesscenter' ),
                'oops'                           => __( 'Something went wrong.', 'wellnesscenter' ),
                'notice_can_install_required'    => _n_noop( 'This theme requires the following plugin: %1$s.', 'This theme requires the following plugins: %1$s.', 'wellnesscenter' ),
                'notice_can_install_recommended' => _n_noop( 'This theme recommends the following plugin: %1$s.', 'This theme recommends the following plugins: %1$s.', 'wellnesscenter' ),
                'notice_cannot_install'          => _n_noop( 'Sorry, but you do not have the correct permissions to install the %s plugin. Contact the administrator of this site for help on getting the plugin installed.', 'Sorry, but you do not have the correct permissions to install the %s plugins. Contact the administrator of this site for help on getting the plugins installed.', 'wellnesscenter' ),
                'notice_can_activate_required'   => _n_noop( 'The following required plugin is currently inactive: %1$s.', 'The following required plugins are currently inactive: %1$s.', 'wellnesscenter' ),
                'notice_can_activate_recommended'=> _n_noop( 'The following recommended plugin is currently inactive: %1$s.', 'The following recommended plugins are currently inactive: %1$s.', 'wellnesscenter' ),
                'notice_cannot_activate'         => _n_noop( 'Sorry, but you do not have the correct permissions to activate the %s plugin. Contact the administrator of this site for help on getting the plugin activated.', 'Sorry, but you do not have the correct permissions to activate the %s plugins. Contact the administrator of this site for help on getting the plugins activated.', 'wellnesscenter' ),
                'notice_ask_to_update'           => _n_noop( 'The following plugin needs to be updated to its latest version to ensure maximum compatibility with this theme: %1$s.', 'The following plugins need to be updated to their latest version to ensure maximum compatibility with this theme: %1$s.', 'wellnesscenter' ),
                'notice_cannot_update'           => _n_noop( 'Sorry, but you do not have the correct permissions to update the %s plugin. Contact the administrator of this site for help on getting the plugin updated.', 'Sorry, but you do not have the correct permissions to update the %s plugins. Contact the administrator of this site for help on getting the plugins updated.', 'wellnesscenter' ),
                'install_link'                   => _n_noop( 'Begin installing plugin', 'Begin installing plugins', 'wellnesscenter' ),
                'activate_link'                  => _n_noop( 'Begin activating plugin', 'Begin activating plugins', 'wellnesscenter' ),
                'return'                         => __( 'Return to Required Plugins Installer', 'wellnesscenter' ),
                'dashboard'                      => __( 'Return to the dashboard', 'wellnesscenter' ),
                'plugin_activated'               => __( 'Plugin activated successfully.', 'wellnesscenter' ),
                'activated_successfully'         => __( 'The following plugin was activated successfully:', 'wellnesscenter' ),
                'complete'                       => __( 'All plugins installed and activated successfully. %1$s', 'wellnesscenter' ),
                'dismiss'                        => __( 'Dismiss this notice', 'wellnesscenter' ),
            );

            // Set the current WordPress version.
            global $wellnesscenter_wp_version;
            $this->wp_version = $wellnesscenter_wp_version;

            // Announce that the class is ready, and pass the object (for advanced use).
            do_action_ref_array( 'wellnesscenter_init', array( $this ) );
         ...
         * @since 2.0.0
         *
         * @see Wellnesscenter_Plugin_Activation::admin_menu()
         * @see Wellnesscenter_Plugin_Activation::notices()
         * @see Wellnesscenter_Plugin_Activation::styles()
         */
        public function init() {

            do_action( 'wellnesscenter_register' );
            ...
            if ( $this->plugins ) {
                ...
                if ( $this->is_wellnesscenter_page() ) {
        ...
        public function admin_init() {
            ...
            if ( isset( $_REQUEST['tab'] ) && 'plugin-information' == $_REQUEST['tab'] ) {
                ...
                global $wellnesscenter_tab, $wellnesscenter_body_id;
                $wellnesscenter_body_id = $wellnesscenter_tab = 'plugin-information';
        ...
        public function thickbox() {
            ...
            if ( ! get_user_meta( get_current_user_id(), 'wellnesscenter_dismissed_notice', true ) ) {
                add_thickbox();
            }
         ...
         *
         * @see Wellnesscenter_Plugin_Activation::init()
         * @see Wellnesscenter_Plugin_Activation::install_plugins_page()
         */
        ...
        public function install_plugins_page() {
            ...
            $plugin_table = new Wellnesscenter_List_Table;

            // Return early if processing a plugin installation action.
            if ( isset( $_POST['action'] ) && 'wellnesscenter-bulk-install' == $_POST['action'] && $plugin_table->process_bulk_actions() || $this->do_plugin_install() ) {
                return;
            }

            ?>
            <div class="wellnesscenter wrap">
                ...
                <form id="wellnesscenter-plugins" action="" method="post">
                    <input type="hidden" name="wellnesscenter-page" value="<?php echo esc_attr($this->menu); ?>" />
        ...
        protected function do_plugin_install() {
            ...
            if ( isset( $_GET['plugin'] ) && ( isset( $_GET['wellnesscenter-install'] ) && 'install-plugin' == $_GET['wellnesscenter-install'] ) ) {
                check_admin_referer( 'wellnesscenter-install' );
                ...
                $url = wp_nonce_url(
                    add_query_arg(
                        array(
                            ...
                            'wellnesscenter-install' => 'install-plugin',
                        ),
                        network_admin_url( 'themes.php' )
                    ),
                    'wellnesscenter-install'
                );
                $method = ''; // Leave blank so WP_Filesystem can populate it as necessary.
                $fields = array( 'wellnesscenter-install' ); // Extra fields to pass to WP_Filesystem.
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . esc_url( network_admin_url() ) . '" title="' . esc_attr__( 'Return to the Dashboard', 'wellnesscenter' ) . '">' . __( 'Return to the Dashboard', 'wellnesscenter' ) . '</a>' ) . '</p>';
            ...
            elseif ( isset( $_GET['plugin'] ) && ( isset( $_GET['wellnesscenter-activate'] ) && 'activate-plugin' == $_GET['wellnesscenter-activate'] ) ) {
                check_admin_referer( 'wellnesscenter-activate', 'wellnesscenter-activate-nonce' );
        ...
        public function notices() {
            ...
            if ( ! empty( $message ) ) {
                ...
                foreach ( $message as $type => $plugin_groups ) {
                    ...
                    $rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[$type], $count, 'wellnesscenter' ), $imploded, $count ) . '</p>';
                }

                // Setup variables to determine if action links are needed.
                $show_install_link  = $install_link ? '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), network_admin_url( 'themes.php' ) ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'wellnesscenter' ) . '</a>' : '';
                $show_activate_link = $activate_link ? '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), network_admin_url( 'themes.php' ) ) ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'wellnesscenter' ) . '</a>'  : '';

                // Define all of the action links.
                $action_links = apply_filters(
                    'wellnesscenter_notice_action_links',
                    array(
                        'install'  => ( current_user_can( 'install_plugins' ) )  ? $show_install_link  : '',
                        'activate' => ( current_user_can( 'activate_plugins' ) ) ? $show_activate_link : '',
                        'dismiss'  => $this->dismissable ? '<a class="dismiss-notice" href="' . esc_url( add_query_arg( 'wellnesscenter-dismiss', 'dismiss_admin_notices' ) ) . '" target="_parent">' . $this->strings['dismiss'] . '</a>' : '',
                    )
                );
                ...
                if ( ! empty( $this->strings['nag_type'] ) ) {
                    add_settings_error( 'wellnesscenter', 'wellnesscenter', $rendered, sanitize_html_class( strtolower( $this->strings['nag_type'] ) ) );
                } else {
                    add_settings_error( 'wellnesscenter', 'wellnesscenter', $rendered, $nag_class );
                }
            }

            // Admin options pages already output settings_errors, so this is to avoid duplication.
            if ( 'options-general' !== $current_screen->parent_base ) {
                settings_errors( 'wellnesscenter' );
            }
        ...
        public function dismiss() {
            ...
            if ( isset( $_GET['wellnesscenter-dismiss'] ) ) {
                update_user_meta( get_current_user_id(), 'wellnesscenter_dismissed_notice', 1 );
            }
        ...
        protected function is_wellnesscenter_page() {
        ...
        public function update_dismiss() {
            ...
            delete_user_meta( get_current_user_id(), 'wellnesscenter_dismissed_notice' );
         ...
         * @since 2.4.0
         *
         * @return object The Wellnesscenter_Plugin_Activation object.
         */
        public static function get_instance() {

            if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Wellnesscenter_Plugin_Activation ) ) {
                self::$instance = new Wellnesscenter_Plugin_Activation();
            }
    ...
    $wellnesscenter = Wellnesscenter_Plugin_Activation::get_instance();

}

if ( ! function_exists( 'wellnesscenter_plugin_activator' ) ) {
    /**
     ...
     */
    function wellnesscenter_plugin_activator( $plugins, $config = array() ) {

        foreach ( $plugins as $plugin ) {
            Wellnesscenter_Plugin_Activation::$instance->register( $plugin );
        }

        if ( $config ) {
            Wellnesscenter_Plugin_Activation::$instance->config( $config );
        }
...
if ( ! class_exists( 'Wellnesscenter_List_Table' ) ) {
    class Wellnesscenter_List_Table extends WP_List_Table {

        public function __construct() {

            global $wellnesscenter_status, $wellnesscenter_page;
        ...
        protected function _gather_plugin_data() {
            ...
            Wellnesscenter_Plugin_Activation::$instance->admin_init();
            Wellnesscenter_Plugin_Activation::$instance->thickbox();
            ...
            foreach ( Wellnesscenter_Plugin_Activation::$instance->plugins as $plugin ) {
                if ( is_plugin_active( $plugin['file_path'] ) ) {
                ...
                if ( ! empty( $plugin['source'] ) ) {
                    ...
                    if ( preg_match( '|^http(s)?://|', $plugin['source'] ) ) {
                        $table_data[$i]['source'] = __( 'Private Repository', 'wellnesscenter' );
                    // The plugin is pre-packaged with the theme.
                    } else {
                        $table_data[$i]['source'] = __( 'Pre-Packaged', 'wellnesscenter' );
                    }
                }
                // The plugin is from the WordPress repository.
                else {
                    $table_data[$i]['source'] = __( 'WordPress Repository', 'wellnesscenter' );
                }

                $table_data[$i]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? __( 'Required', 'wellnesscenter' ) : __( 'Recommended', 'wellnesscenter' );

                if ( ! isset( $installed_plugins[$plugin['file_path']] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', 'wellnesscenter' ) );
                } elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'wellnesscenter' ) );
                }
         ...
         * Wellnesscenter_Plugin_Activation class.
         *
         * Loops through the registered plugins looking for $name. If it finds it,
         * it returns the $data from that plugin. Otherwise, returns false.
         *
        ...
        protected function _get_plugin_data_from_name( $name, $data = 'slug' ) {
            ...
            foreach ( Wellnesscenter_Plugin_Activation::$instance->plugins as $plugin => $values ) {
                if ( $name == $values['name'] && isset( $values[$data] ) ) {
                    return $values[$data];
                }
            }
        ...
        public function column_plugin( $item ) {
            ...
            if ( ! isset( $installed_plugins[$item['file_path']] ) ) {
                $actions = array(
                    'install' => sprintf(
                        '<a href="%1$s" title="' . esc_attr__( 'Install', 'wellnesscenter' ) . ' %2$s">' . __( 'Install', 'wellnesscenter' ) . '</a>',
                        esc_url(
                            wp_nonce_url(
                                add_query_arg(
                                    array(
                                        'page'          => urlencode( Wellnesscenter_Plugin_Activation::$instance->menu ),
            ...
            elseif ( is_plugin_inactive( $item['file_path'] ) ) {
                $actions = array(
                    'activate' => sprintf(
                        '<a href="%1$s" title="' . esc_attr__( 'Activate', 'wellnesscenter' ) . ' %2$s">' . __( 'Activate', 'wellnesscenter' ) . '</a>',
                        esc_url(
                            add_query_arg(
                                array(
                                    'page'                 => urlencode( Wellnesscenter_Plugin_Activation::$instance->menu ),
                                    ...
                                    'plugin_name'          => urlencode( $item['sanitized_plugin'] ),
                                    'plugin_source'        => urlencode( $item['url'] ),
                                    'wellnesscenter-activate'       => 'activate-plugin',
                                    'wellnesscenter-activate-nonce' => urlencode( wp_create_nonce( 'wellnesscenter-activate' ) ),
                                ),
                                network_admin_url( 'themes.php' )
                            )
                        ),
        ...
        public function column_cb( $item ) {
            ...
            if ( __( 'Private Repository', 'wellnesscenter' ) === $item['source'] ) {
                // Escape external URLs
                $plugin_url = esc_url( $plugin_url );
            } elseif ( __( 'Pre-Packaged', 'wellnesscenter' ) === $item['source'] ) {
                // Encode file path for use in attribute
                $plugin_url = urlencode( Wellnesscenter_Plugin_Activation::$instance->default_path . $plugin_url );
            }
        ...
        public function no_items() {
            ...
            printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', 'wellnesscenter' ), network_admin_url() );
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        ...
        public function get_columns() {
            ...
            $columns = array(
                ...
                'plugin' => __( 'Plugin', 'wellnesscenter' ),
                'source' => __( 'Source', 'wellnesscenter' ),
                'type'   => __( 'Type', 'wellnesscenter' ),
                'status' => __( 'Status', 'wellnesscenter' )
            );
        ...
        public function get_bulk_actions() {
            ...
            $actions = array(
                'wellnesscenter-bulk-install'  => __( 'Install', 'wellnesscenter' ),
                'wellnesscenter-bulk-activate' => __( 'Activate', 'wellnesscenter' ),
            );
        ...
        public function process_bulk_actions() {
            ...
            if ( 'wellnesscenter-bulk-install' === $this->current_action() ) {
                ...
                $url = wp_nonce_url(
                    add_query_arg(
                        array(
                            'page'          => urlencode( Wellnesscenter_Plugin_Activation::$instance->menu ),
                            'wellnesscenter-action'  => 'install-selected',
                ...
                // Loop through each plugin to install and try to grab information from WordPress API, if not create 'wellnesscenter-empty' scalar.
                $i = 0;
                foreach ( $plugin_installs as $plugin ) {
                    $api[$i] = plugins_api( 'plugin_information', array( 'slug' => $plugin, 'fields' => array( 'sections' => false ) ) ) ? plugins_api( 'plugin_information', array( 'slug' => $plugin, 'fields' => array( 'sections' => false ) ) ) : (object) $api[$i] = 'wellnesscenter-empty';
                    $i++;
                }

                if ( is_wp_error( $api ) ) {
                    wp_die( Wellnesscenter_Plugin_Activation::$instance->strings['oops'] . var_dump( $api ) );
                }
                ...
                $url   = esc_url_raw( add_query_arg( array( 'page' => urlencode( Wellnesscenter_Plugin_Activation::$instance->menu ) ), network_admin_url( 'themes.php' ) ) );
                $nonce = 'bulk-plugins';
                $names = $plugin_names;

                // Create a new instance of Wellnesscenter_Bulk_Installer.
                $installer = new Wellnesscenter_Bulk_Installer( $skin = new Wellnesscenter_Bulk_Installer_Skin( compact( 'url', 'nonce', 'names' ) ) );

                // Wrap the install process with the appropriate HTML.
                echo '<div class="wellnesscenter wrap">';
            ...
            if ( 'wellnesscenter-bulk-activate' === $this->current_action() ) {
                check_admin_referer( 'bulk-' . $this->_args['plural'] );
                ...
                if ( is_wp_error( $activate ) ) {
                    ...
                    printf( '<div id="message" class="updated"><p>%1$s %2$s.</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'wellnesscenter' ), $imploded );
                }
...
add_action( 'admin_init', 'wellnesscenter_load_bulk_installer' );
if ( ! function_exists( 'wellnesscenter_load_bulk_installer' ) ) {
    function wellnesscenter_load_bulk_installer() {
    /**
     ...
     * @since 2.2.0
     */
    if ( ! class_exists( 'WP_Upgrader' ) && ( isset( $_GET['page'] ) && Wellnesscenter_Plugin_Activation::$instance->menu === $_GET['page'] ) ) {
        require_once ABSPATH . 'wp-admin/includes/class-wp-upgrader.php';

        if ( ! class_exists( 'Wellnesscenter_Bulk_Installer' ) ) {
            class Wellnesscenter_Bulk_Installer extends WP_Upgrader {
                ...
                public function bulk_install( $packages ) {
                    ...
                    if ( Wellnesscenter_Plugin_Activation::$instance->is_automatic ) {
                ...
                public function run( $options ) {
                    ...
                    if ( ! $res ) {
                        ...
                        if ( Wellnesscenter_Plugin_Activation::$instance->is_automatic ) {
                            ...
                            Wellnesscenter_Plugin_Activation::$instance->populate_file_path();
                    ...
                    public function install_strings() {
                        ...
                        $this->strings['no_package']          = __( 'Install package not available.', 'wellnesscenter' );
                        $this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', 'wellnesscenter' );
                        $this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', 'wellnesscenter' );
                        $this->strings['installing_package']  = __( 'Installing the plugin&#8230;', 'wellnesscenter' );
                        $this->strings['process_failed']      = __( 'Plugin install failed.', 'wellnesscenter' );
                        $this->strings['process_success']     = __( 'Plugin installed successfully.', 'wellnesscenter' );
                    ...
                    public function activate_strings() {
                        ...
                        $this->strings['activation_failed']  = __( 'Plugin activation failed.', 'wellnesscenter' );
                        $this->strings['activation_success'] = __( 'Plugin activated successfully.', 'wellnesscenter' );
            ...
            if ( ! class_exists( 'Wellnesscenter_Bulk_Installer_Skin' ) ) {
                class Wellnesscenter_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
                    ...
                    public function add_strings() {
                        ...
                        if ( Wellnesscenter_Plugin_Activation::$instance->is_automatic ) {
                            $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'wellnesscenter' );
                            $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'wellnesscenter' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'wellnesscenter' ) . '</span><span class="hidden">' . __( 'Hide Details', 'wellnesscenter' ) . '</span>.</a>';
                            $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', 'wellnesscenter' );
                            $this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'wellnesscenter' );
                        }
                        // Default installation strings.
                        else {
                            $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'wellnesscenter' );
                            $this->upgrader->strings['skin_update_failed_error']  = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'wellnesscenter' );
                            $this->upgrader->strings['skin_update_failed']        = __( 'The installation of %1$s failed.', 'wellnesscenter' );
                            $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', 'wellnesscenter' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'wellnesscenter' ) . '</span><span class="hidden">' . __( 'Hide Details', 'wellnesscenter' ) . '</span>.</a>';
                            $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', 'wellnesscenter' );
                            $this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', 'wellnesscenter' );
                        }
                    ...
                    public function bulk_footer() {
                        ...
                        foreach ( Wellnesscenter_Plugin_Activation::$instance->plugins as $plugin ) {
                            if ( ! is_plugin_active( $plugin['file_path'] ) ) {
                                echo '<p><a href="' . esc_url( add_query_arg( 'page', urlencode( Wellnesscenter_Plugin_Activation::$instance->menu ), network_admin_url( 'themes.php' ) ) ) . '" title="' . esc_attr( Wellnesscenter_Plugin_Activation::$instance->strings['return'] ) . '" target="_parent">' . Wellnesscenter_Plugin_Activation::$instance->strings['return'] . '</a></p>';
                                $complete[] = $plugin;
                                break;
                            }
                            // Nothing to store.
                        ...
                        if ( empty( $complete ) ) {
                            echo '<p>' .  sprintf( Wellnesscenter_Plugin_Activation::$instance->strings['complete'], '<a href="' . esc_url( network_admin_url() ) . '" title="' . esc_attr__( 'Return to the Dashboard', 'wellnesscenter' ) . '">' . __( 'Return to the Dashboard', 'wellnesscenter' ) . '</a>' ) . '</p>';
                            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                        }
