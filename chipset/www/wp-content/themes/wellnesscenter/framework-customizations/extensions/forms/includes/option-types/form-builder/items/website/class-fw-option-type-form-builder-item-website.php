...
class Wellnesscenter_Option_Type_Form_Builder_Item_Website extends FW_Option_Type_Form_Builder_Item {
	...
	public function get_thumbnails() {
		return array(
			array(
				'html' =>
					'<div class="item-type-icon-title" data-hover-tip="' . __( 'Add a Website field', 'wellnesscenter' ) . '">' .
					'<div class="item-type-icon">' .
					'<img src="' . esc_url( $this->get_uri( '/static/images/icon.png' ) ) . '" />' .
					'</div>' .
					'<div class="item-type-title">' . __( 'Website', 'wellnesscenter' ) . '</div>' .
					'</div>'
			)
		);
	}
	...
	public function enqueue_static() {
		...
		wp_localize_script(
			...
			array(
				'l10n'     => array(
					'item_title'      => __( 'Website', 'wellnesscenter' ),
					...
					'options' => array(
						array(
							'label' => array(
								...
								'value' => __( 'Website', 'wellnesscenter' ),
							)
						),
						array(
							'required' => array(
	...
	public function frontend_validate( array $item, $input_value ) {
		...
		$messages = array(
			...
			'incorrect' => str_replace(
				array( '{label}' ),
				array( $options['label'] ),
				__( 'The {label} field must be a valid website name', 'wellnesscenter' )
			),
		);
...
FW_Option_Type_Builder::register_item_type( 'Wellnesscenter_Option_Type_Form_Builder_Item_Website' );
