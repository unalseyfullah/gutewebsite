        ...
        foreach ( $atts['tabs'] as $tab ) : ?>
            <div id="<?php echo   $id.'-'.$cnt;?>" class="tab-pane fade <?php echo ($cnt == 1) ? 'in active' : '';?>">
                <?php if(!empty($tab['tab_content_title'])):?>
                    <strong><?php echo wellnesscenter_theme_translate(esc_html($tab['tab_content_title'])); ?></strong>
                <?php endif;?>

                <?php if(!empty($tab['tab_content'])):?>
                    <?php echo wellnesscenter_theme_translate(esc_textarea(do_shortcode($tab['tab_content']) ));?>
                <?php endif;?>
            </div>
        <?php $cnt++; endforeach; ?>
    </div>
